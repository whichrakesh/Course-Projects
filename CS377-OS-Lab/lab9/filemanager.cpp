...
#define error(X) { unique_lock<mutex> locker(mutex_cout); cerr << X<<endl; locker.unlock();}
#define min(X,Y) (X<Y?X:Y)
#define max(X,Y) (X>Y?X:Y)

using namespace std;

mutex mutex_disk,mutex_cout;
condition_variable cond_file_open;
...
public:
	...
	void makeDirectory(string uname, string dir_name){
		if(userExists(uname)){
			...
			println("new directory " << dir_name << " added to dir "<< user->current_directory);
	...
	int openFile(string uname, string file_path, FCB &fcb){
		...
		if(addr == -1){
			...
			if(res == -1) {
				...
				cond_file_open.wait(disk_locker,[this,&cur_dir_addr,&cur_dir,&file_name](){
					char block[BLOCK_SIZE];
					this->disk.read(cur_dir_addr,block);
					cur_dir = Directory(string(block));
					int ret = cur_dir.lockFile(file_name);
					return ret == 0;
				});				
			} else if(res == -2){
				error("no such file:" << fcb.file_name << " to open.");
				disk_locker.unlock();
				return -2;
			}
			stringToCharArray(cur_dir.toString(),block,'\0');
			disk.write(cur_dir_addr,block);			
			fcb = FCB(file_name,0,addr,cur_dir_addr);
			println("opened file:" << file_name << " for processing");
			disk_locker.unlock();
			return 0;
		}
	}
	int writeFile(FCB &fcb, int bytes_count, string value){
		...
		println("wrote value " << value << " in file " << fcb.file_name << " at block " << block_no << " byte " << byte_no);
	...
	int fileSeek(FCB &fcb, int seek){		
		...
		if(seek < file_size){ // no need to append space
			debug("file "<< fcb.file_name << " is seeked to byte no. " << seek);					
			return 0;
		}
		int next_block_no = fcb.file_addr;//dummy
		int prev_block_no = 0;
	...
	int newFileBlock(int prev_block_no){
		...
		println("new file block with address " << next_block_no << " created.");
	...
	int closeFile(FCB &fcb){
		...
		stringToCharArray(dir.toString(),block,'\0');
		disk.write(fcb.parent_dir_addr,block);
		println("file:" << fcb.file_name << " closed.");
		cond_file_open.notify_all();
	}
}fs;
