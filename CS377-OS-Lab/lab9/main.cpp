...
// #define println(X) { unique_lock<mutex> locker(mutex_cout); cout << X<<endl; locker.unlock();}
// bool DEBUG = false;
int RAM_size, l_thresh, h_thresh;
static long timer;

mutex mutex_io_table, mutex_free_list, mutex_lru_table, mutex_pt;
...
struct PageTable{
    ...
    void print_pt_table(){
        ...
        for(int i = 0; i < size_; i++){
            ...
            println(i<<"\t |"<<temp_p.frame_no<<"\t |"<<temp_p.valid<<"\t |"<<temp_p.dirty);
        }
    }
};
...
int main(){
    ...
    while(getline(fin,line)){
        ...
        if(command == "end"){
            ...
            if(command == "create"){
                string user_name = words[1]; //uname
                if(!fs.userExists(user_name)){
                    error("no such user with name "<< user_name << " exists.");
                    continue;
                }
                int pid = stoi(words[2]);  //pid
                PInfo pi;
                pi.user = user_name;
                pi.process_size = stoi(words[3]);  // size of process
                pdata[pid] = pi;
                int b = 2;
                pthreads.push_back(thread(processT,pid));
                list_of_pid.push_back(pid);
            } else if(command == "Memory_size"){
                RAM_size = stoi(words[1]);
                ...
                l_thresh = stoi(words[1]);
            } else if(command == "Upper_threshold"){
                h_thresh = stoi(words[1]);
                ...
                for(int i = 0; i < list_of_pid.size(); i++){
                    ...
                    println("page no | frame no | valid | dirty");
                    pt->print_pt_table();
                }

            } else if(command == "uname"){
                fs.checkUser(words[1]);
            } else if(command == "mkdir"){
                fs.makeDirectory(words[1],words[2]);
            } else if(command == "cd"){
                fs.changeCurrentDirectory(words[1],words[2]);
            } else {
...
void processT(int pid){
    string user_name = pdata[pid].user;
    User* user = fs.getUser(user_name);
    map<string,FCB> oft;
    FCB fcb;
    string cur_dir = user->current_directory;
    ...
    while (infile >> command){
        if(command == "end") {
            this_thread::sleep_for(chrono::seconds(1));
            println("--------------------table for "<<pid<<"--------------------------");
        	println("Number of access operations : " << formatNumber(accessed));
        	println("Number of modify operations : " << formatNumber(modified));
        	println("Number of page faults : " << formatNumber(pagefaults));
            //cout<<"---------------------------------------------------------\n";
            for(auto it = oft.begin(); it != oft.end(); it++){
                fs.closeFile(it->second);
            }
            return;
            ...
            int ret = fs.openFile(user_name,file_name,fcb);
            if(ret == 0){
                oft[file_name] = fcb;
            }
            ...
            infile >> file_name;            
            file_name = cur_dir + file_name;
            if(oft.count(file_name)){
                string relative;
                infile >> relative;
                int seek_pos;
                if(relative == "relative"){
                    infile >> seek_pos;
                    seek_pos = oft[file_name].cur_position + seek_pos;                            
                }
                else
                    seek_pos = stoi(relative);
                fs.fileSeek(oft[file_name],seek_pos);
            } else {
                error("no such file open:" << file_name);
            }
            ...
            file_name = cur_dir + file_name;
            if(oft.count(file_name)){
                string data = fs.readFile(oft[file_name],bytes_count);
                println("data got from file read: " << data);
            } else {
                error("no such file open:" << file_name);
            }
            ...
            file_name = cur_dir + file_name;
            if(oft.count(file_name)){
                fs.writeFile(oft[file_name],bytes_count,data);
            } else {
                error("no such file open:" << file_name);
            }
        }
            ...
            println("Process "<<pid << ": attempted to access/modify " << page_no << " " << word_no);
            if(command == "access"){
            ...
            if(response == -1){
                println("Reported a memory protection violation ");
            }else if(response == -2){
                println("Reported a page fault");
                pagefaults++;
            }else{
                println("Accessed page frame number "<< response);
            }
        }
    }
}
