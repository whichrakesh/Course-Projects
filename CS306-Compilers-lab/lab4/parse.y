...
statement
  ...
  | RETURN expression ';'
      ...
      Type *return_type = gst->table[lst->func_name].return_type;
        ...
        if(return_type->tag == Type::Base &&  return_type->basetype == Type::Int){
          $$ = new ReturnStmtAst(new ToInt($2));          
        } else if(return_type->tag == Type::Base &&  return_type->basetype == Type::Float){
      ...
      if(gst->table.count($1) > 0){  
        ...
        for(auto it = $3->begin(); it != $3->end(); it++){            
            ...
            if(!compatible((*it)->getType(),t)){
              parse_error( "Function " << $1 << "'s parameters not compatible with defination.");
              $$->setType(new Type(Type::Error));
              return;
            }
            i++;    
        ...
        if(i != temp->parameters.size()){
          parse_error( "Function call " << $1 << " has less parameters than the defination.");
          $$->setType(new Type(Type::Error));
        }
        $$->setType(new Type(Type::Ok));
      }else{
